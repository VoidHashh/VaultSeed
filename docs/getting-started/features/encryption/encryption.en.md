...
### In General
In **Settings -> Encryption Settings**, you can adjust *PBKDF2 Iterations* and *Encryption Mode* before loading a wallet. When encrypting, VaultSeed applies these preferences to optimize for the specific secret being secured, producing an encrypted KEF envelope that can be exported as a QR or saved to SD.
...
There are several ways to add security layers to a wallet’s private key. The most common is adding a BIP39 passphrase to the mnemonic. Encrypting a BIP39 mnemonic serves a similar purpose, but differs in a key way: with BIP39 passphrases, entering the wrong passphrase loads a different wallet, while with VaultSeed’s encrypted mnemonics, a wrong key simply returns an error. Depending on the use case, this behavior may be preferable. VaultSeed’s implementation also stores a mnemonic ID alongside the data for convenience. Encrypted mnemonics can be combined with a BIP39 passphrase for added security, and the same mnemonic can be encrypted with multiple different keys.

## AES Modes-of-Operation

VaultSeed uses standard AES encryption with modes-of-operation: ECB, CBC, CTR and GCM. The user may set their preference within `Settings, Encryption Settings, Encryption Mode`. VaultSeed uses GCM as the default mode-of-operation, but you may have valid reasons for making your own choice, ie:
...
ECB (Electronic Codebook) is a simpler method where data blocks are encrypted individually. This mode is faster and simpler to encrypt, resulting in QR codes with lower density that are easier to [transcribe](../QR-transcript-tools.md). It is generally considered less secure than others because it does not provide data chaining, meaning identical plaintext blocks would produce identical ciphertext blocks, making it vulnerable to pattern analysis. However, in VaultSeed's implementation, encrypting plaintext via ECB which contains duplicate blocks has been intentionally disabled.
